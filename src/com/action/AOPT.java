...
public class AOPT {
	...
	final static Logger logger = Logger.getLogger(AOPT.class); 
...
String requestString ="{\"P_REGISTRATION_ID\":\""+P_REGISTRATION_ID+"\",\"P_SR_NUMBER\":\""+P_SR_NUMBER+"\"}";
	...
	public String PaymentPlanReversalMultiple(String P_REGISTRATION_ID, String P_SR_NUMBER) {
		JSONObject result = null;
		try {
			result = new JSONObject();
			JSONObject resultSub = null;
			JSONArray arr = new JSONArray();
			String P_REGISTRATION_IDLen[] = P_REGISTRATION_ID.split(",");
			String P_SR_NUMBERLen[] = P_SR_NUMBER.split(",");
			for(int i=0;i<P_REGISTRATION_IDLen.length;i++) {
				resultSub = new JSONObject();
			final String queueName = "IPMS.AOPT.PaymentPlanReversal";
			String jsonString ="{\"P_REGISTRATION_ID\":\""+P_REGISTRATION_IDLen[i]+"\",\"P_SR_NUMBER\":\""+P_SR_NUMBERLen[i]+"\"}";
			logger.info("AOPTPaymentPlanReversal :: "+jsonString);
			activemqcall=new ActiveMQCall();
			String correlationId = activemqcall.createRandomString();
			System.out.println("1 :: "+correlationId);	
			correlationId=activemqcall.GetProducer(queueName,P_REGISTRATION_IDLen[i],correlationId,jsonString);
			System.out.println("2 :: "+correlationId);
			...
			SOAHeader header = new SOAHeader();
	   		com.oracle.xmlns.apps.ont.soaprovider.plsql.xxdc_aopt_pkg_ws.payment_plan_reversal.InputParameters aoptplanreversal = new com.oracle.xmlns.apps.ont.soaprovider.plsql.xxdc_aopt_pkg_ws.payment_plan_reversal.InputParameters();
			aoptplanreversal.setP_REGISTRATION_ID(P_REGISTRATION_IDLen[i]);
			aoptplanreversal.setP_SR_NUMBER(P_SR_NUMBERLen[i]);
			com.oracle.xmlns.apps.ont.soaprovider.plsql.xxdc_aopt_pkg_ws.payment_plan_reversal.OutputParameters aoptplanreversalout = port_aopt.PAYMENT_PLAN_REVERSAL(header, aoptplanreversal);
			System.out.println(aoptplanreversalout.getX_RETURN_STATUS());
			logger.info("AOPTPaymentPlanReversal response status :: "+aoptplanreversalout.getX_RETURN_STATUS());
			resultSub.put("status", aoptplanreversalout.getX_RETURN_STATUS());
			resultSub.put("message", aoptplanreversalout.getX_ERROR_MSG());
			resultSub.put("P_REGISTRATION_ID", P_REGISTRATION_IDLen[i]);
			resultSub.put("P_SR_NUMBERLen", P_SR_NUMBERLen[i]);
			arr.put(resultSub);
...
//			String correlationId = P_REQUEST_NUMBER;
	...
	public  String DocumentAttachmentMultiple1(String P_REQUEST_NUMBER,String P_REQUEST_NAME,String P_SOURCE_SYSTEM, com.bean.DocUploadDTO[] regTerms) {
		//process
...
//	   		
	   		...
	   		for(int i=0;i<objAPPSXXDC_PROCESS_SEX1794747X1X57.length;i++) {
	   			...
	   			if(objAPPSXXDC_PROCESS_SEX1794747X1X57[i].getATTRIBUTE1() != null) {
	   				subJson.put("URL", objAPPSXXDC_PROCESS_SEX1794747X1X57[i].getATTRIBUTE1());
	   			}else {
	   				subJson.put("URL", JSONObject.NULL);
	   			}
	   				...
	   				if(objAPPSXXDC_PROCESS_SEX1794747X1X57[i].getPROC_STATUS().equals("S")) {
	   				   	...
	   				   	if(objAPPSXXDC_PROCESS_SEX1794747X1X57[i].getATTRIBUTE1() != null) {
	   				   	String fileName = objAPPSXXDC_PROCESS_SEX1794747X1X57[i].getATTRIBUTE1().substring(objAPPSXXDC_PROCESS_SEX1794747X1X57[i].getATTRIBUTE1().lastIndexOf('/')+1, objAPPSXXDC_PROCESS_SEX1794747X1X57[i].getATTRIBUTE1().length());
	   				   	//objFtp.transferFileToIPMS(fileName,regTerms[i].getFileBinary());
	   				 new Thread(new Runnable(){

	   			      public void run(){
	   			    	//objFtp.transferFileToIPMSFTPS(P_REQUEST_NUMBER,fileName,regTerms[k].getFileBinary());
	   			    	FtpClientIPMS.IPMSUploadBase64(P_REQUEST_NUMBER,fileName,regTerms[k].getFileBinary());
	   			      }  

	   				 }).start();
	   				   	
	   				 //subJson.put("FTP_Response", ftpRes);
	   				   	}
	   			    }
	   		 
	   			arrJ.put(subJson);
	   			String requestString="{\"P_REQUEST_NUMBER\":\""+P_REQUEST_NUMBER+"\",\"P_REQUEST_NAME\":\""+P_REQUEST_NAME+"\",\"P_SOURCE_SYSTEM\":\""+P_SOURCE_SYSTEM+"\",\"PARAM_ID\":\""+regTerms[i].getSourceId()+"\",\"ATTRIBUTE1\":\""+regTerms[i].getRegistrationId()+"\",\"ATTRIBUTE2\":\""+regTerms[i].getCategory()+"\",\"ATTRIBUTE3\":\""+regTerms[i].getFileId()+"\",\"ATTRIBUTE4\":\""+regTerms[i].getFileName()+"\",\"ATTRIBUTE5\":\""+regTerms[i].getFileDescription()+"\",\"ATTRIBUTE6\":\""+regTerms[i].getSourceFileName()+"\"}";
...
String requestString ="{\"P_REGISTRATION_ID\":\""+REGISTRATION_ID+"\"}";
	...
	public String getMilestonePaymentDetails( String REGISTRATION_ID) {
		JSONObject result = new JSONObject();
		final String queueName = "IPMS.AOPT.MilestonePaymentDetails";
		//logger.info("queueName :: "+queueName);
//http://localhost:8080/CRM_SR_NEW/services/AOPT/AOPTRegistrationDetails?P_REGISTRATION_ID=10745&P_SR_NUMBER=123
			//"SRNumber":""+SRNumber+"",
		String jsonString ="{\"P_REGISTRATION_ID\":\""+REGISTRATION_ID+"\"}";
